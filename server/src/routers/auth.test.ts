...
import { jwtExpires } from "../configs/server.config";
...
describe("POST /auth", () => {
  test("should respond as expected", async () => {
    ...
    expect(response.body.data.expiresIn).toEqual(jwtExpires);
  });

  test("should response 401 error with wrong pass", async () => {
    const response = await request(server).post("/login").send(fakeIncorrect);
...
describe("GET /auth", () => {
  test("should respond as expected", async () => {
    const response = await request(server).get("/login").send(fakeCorrect);
