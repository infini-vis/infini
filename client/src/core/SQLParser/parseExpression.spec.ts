...
test('=,<>,<,>,<=,>=', () => {
  ...
  expect(parseExpression({ type: '<>', left: '1', right: '2' })).toBe(
    `1 <> '2'`
  );
  expect(parseExpression({ type: '<', left: '1', right: '2' })).toBe(`1 < '2'`);
  expect(parseExpression({ type: '>=', left: '1', right: '2' })).toBe(
...
test('between, not between', () => {
  expect(
    parseExpression({ type: 'between', field: 'col', left: '1', right: '2' })
  ).toBe(`col BETWEEN '1' AND '2'`);
  expect(
    parseExpression({
      type: 'not between',
      field: 'col',
  ...
  ).toBe(`col NOT BETWEEN '1' AND '2'`);
});

test('is null, is not null', () => {
  expect(parseExpression({ type: 'is null', field: 'col' })).toBe(
    `col IS NULL`
  );
  expect(parseExpression({ type: 'is not null', field: 'col' })).toBe(
    `col IS NOT NULL`
  );
});
...
test('ilike, like, not like', () => {
  ...
  expect(parseExpression({ type: 'like', left: 'col', right: 'infini' })).toBe(
    `col LIKE '%infini%'`
  );
  expect(
    parseExpression({ type: 'not like', left: 'col', right: 'infini' })
  ).toBe(`col NOT LIKE '%infini%'`);
});
...
test('in, not in', () => {
  ...
  expect(
    parseExpression({ type: 'in', set: ['1', '2', '3', '4'], expr: 'col' })
  ).toBe(`col IN ('1', '2', '3', '4')`);
  expect(
    parseExpression({ type: 'not in', set: [1, 2, 3, 4], expr: 'col' })
  ).toBe(`col NOT IN (1, 2, 3, 4)`);
  expect(
    parseExpression({ type: 'not in', set: ['1', '2', '3', '4'], expr: 'col' })
  ).toBe(`col NOT IN ('1', '2', '3', '4')`);
});
...
test('not', () => {
  expect(
    parseExpression({
      type: 'not',
      expr: { type: 'in', set: [1, 2, 3, 4], expr: 'col' },
    })
  ).toBe(`NOT(col IN (1, 2, 3, 4))`);
});

test('and, or', () => {
  expect(parseExpression({ type: 'and', left: 'col1', right: 'col2' })).toBe(
    `(col1 AND col2)`
  );
  expect(parseExpression({ type: 'or', left: 'col1', right: 'col2' })).toBe(
    `(col1 OR col2)`
  );
...
test('date_trunc', () => {
  expect(
    parseExpression({ type: 'date_trunc', unit: 'month', field: 'col' })
  ).toBe(`date_trunc('month', col)`);
});

test('extract', () => {
  expect(
    parseExpression({ type: 'extract', unit: 'month', field: 'col' })
  ).toBe(`extract(month from col)`);
});
...
test('stddev, stddev_pop, stddev_samp, var_pop, var_samp', () => {
  ...
  expect(
    parseExpression({ type: 'stddev', expr: { type: 'count', field: 'col' } })
  ).toBe(`stddev(count(col))`);
  expect(parseExpression({ type: 'stddev_pop', expr: 'col' })).toBe(
    `stddev_pop(col)`
  );
  expect(parseExpression({ type: 'stddev_samp', expr: 'col' })).toBe(
    `stddev_samp(col)`
  );
  expect(parseExpression({ type: 'var_pop', expr: 'col' })).toBe(
    `var_pop(col)`
  );
  expect(parseExpression({ type: 'var_samp', expr: 'col' })).toBe(
    `var_samp(col)`
  );
});
...
test('corr, covar_pop, covar_samp', () => {
  ...
  expect(parseExpression({ type: 'covar_pop', x: 1, y: 2 })).toBe(
    `covar_pop(2, 1)`
  );
  expect(parseExpression({ type: 'covar_samp', x: 1, y: 2 })).toBe(
    `covar_samp(2, 1)`
  );
  expect(parseExpression({ type: 'regr_avgx', x: 1, y: 2 })).toBe(
    `regr_avgx(2, 1)`
  );
  expect(parseExpression({ type: 'regr_avgy', x: 1, y: 2 })).toBe(
    `regr_avgy(2, 1)`
  );
  expect(parseExpression({ type: 'regr_count', x: 1, y: 2 })).toBe(
    `regr_count(2, 1)`
  );
  expect(parseExpression({ type: 'regr_intercept', x: 1, y: 2 })).toBe(
    `regr_intercept(2, 1)`
  );
  expect(parseExpression({ type: 'regr_r2', x: 1, y: 2 })).toBe(
    `regr_r2(2, 1)`
  );
  expect(parseExpression({ type: 'regr_slope', x: 1, y: 2 })).toBe(
    `regr_slope(2, 1)`
  );
  expect(parseExpression({ type: 'regr_sxx', x: 1, y: 2 })).toBe(
    `regr_sxx(2, 1)`
  );
  expect(parseExpression({ type: 'regr_sxy', x: 1, y: 2 })).toBe(
    `regr_sxy(2, 1)`
  );
  expect(parseExpression({ type: 'regr_syy', x: 1, y: 2 })).toBe(
    `regr_syy(2, 1)`
  );
});
...
test('avg, bit_and, bit_or, bool_and, bool_or, every, max, sum', () => {
  ...
  expect(parseExpression({ type: 'bit_and', field: 'col' })).toBe(
    `bit_and(col)`
  );
  expect(parseExpression({ type: 'bit_or', field: 'col' })).toBe(`bit_or(col)`);
  expect(parseExpression({ type: 'bool_and', field: 'col' })).toBe(
    `bool_and(col)`
  );
  expect(parseExpression({ type: 'bool_or', field: 'col' })).toBe(
    `bool_or(col)`
  );
